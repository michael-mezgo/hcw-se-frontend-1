...
describe("Booking (CarRentalPage) component", () => {
  ...
  it("renders View Details link for each car", () => {
    cy.intercept("GET", "/api/cars*", { body: [mockCars[0]] }).as("getCars");
    mountBooking();
    cy.wait("@getCars");
  ...
  it("shows error state when fetch fails", () => {
    cy.intercept("GET", "/api/cars*", { statusCode: 500, body: "Error" }).as("getCars");
    mountBooking();
    cy.wait("@getCars");
  ...
  it("shows all cars (including unavailable) after toggling filter", () => {
    cy.intercept("GET", "/api/cars?available=true", { body: [mockCars[0]] }).as("availableOnly");
    mountBooking();
    cy.wait("@availableOnly");
    cy.intercept("GET", "/api/cars", { body: mockCars }).as("allCars");
  ...
  it("shows no cars when list is empty", () => {
    cy.intercept("GET", "/api/cars*", { body: [] }).as("getCars");
    mountBooking();
    cy.wait("@getCars");
